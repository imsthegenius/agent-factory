...
_Avoid_: "NARUKAMI", "the bot", "Claude" (too specific -- agent is swappable)
...
**Task**:
A work item from the **issue provider** that the **agent** selects and works on during an **iteration**.
...
- **Init** creates the **config directory** on the **host**, prompting the user to select an **agent** and **issue provider**
