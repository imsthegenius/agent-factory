...
3. [Scaffold integration](#scaffold-integration) — what `factory init` needs to offer the agent.
...
- **Non-interactive run mode.** A flag (often `--print`, `exec`, `run`) that takes a prompt, runs the agent to completion, and exits. Without this, Agent Factory cannot drive the agent unattended.
...
- **Persisted session storage.** The agent writes its conversation record somewhere addressable by session ID — a JSONL file, a SQLite row, etc. — so Agent Factory can transfer it between **host** and **sandbox**.

If any of these is missing, the agent likely cannot be supported until its CLI changes.

### Optional capabilities

These unlock extra Agent Factory features but are not required:
...
For `factory init` to offer the agent:
...
- `captureSessions` — user-facing kill-switch. When `true` (default), Agent Factory records the agent's session log per **iteration** and is able to resume it. Expose this on the provider's `Options` interface so users can opt out.
...
- `parseStreamLine(line)` — given one line of stdout, return zero or more `ParsedStreamEvent`s. Event types: `text`, `result`, `tool_call`, `session_id`. Return `[]` for lines you can't or don't need to parse. **Emitting `session_id` is required** — without it, Agent Factory cannot capture the session for resume.
...
For the agent to appear in `factory init`, add an entry to `AGENT_REGISTRY` in [`src/InitService.ts`](../../src/InitService.ts):
