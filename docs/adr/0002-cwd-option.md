...
`promptFile` is **not** re-rooted under `cwd`. It is resolved as an ordinary Node file path, against `process.cwd()`. Only host-repo-derived paths (`.factory/worktrees/`, `.factory/.env`, `.factory/logs/`, `.factory/patches/`) follow `cwd`.

The rejected alternative was to resolve relative `promptFile` against `cwd`, so `"./.factory/prompt.md"` would always mean "the prompt inside the factory project." That reads natural in examples, but it makes `promptFile` behave unlike every other file path a user passes to a Node API — surprising when debugging.
