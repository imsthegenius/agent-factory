...
Lock files live in the **config directory** at `.factory/locks/<name>.lock`, where `<name>` matches the **worktree** directory name under `.factory/worktrees/<name>/`. This keeps locks separate from **worktree** contents (invisible to the **agent**) and provides a 1:1 mapping between **worktrees** and locks.
...
- **Lock inside the worktree** (e.g. `<worktree-path>/.factory.lock`). Visible to the **agent**, which could delete or commit it. Would require `.gitignore` management.
...
- The `.factory/locks/` directory becomes a new managed directory in the **config directory**.
